#!/usr/bin/env python3
"""
MedAgg Healthcare POC - Deployed Backend
...
# Public URL - This will be updated when deployed
...
class DeployedAIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            if self.path == '/':
                ...
                self.end_headers()
                self.wfile.write(b'<h1>MedAgg Healthcare POC - Deployed AI Backend</h1><p>Backend with Conversational AI is running!</p>')
...
def start_deployed_backend():
    """Start the deployed backend server"""
    init_dummy_data()
    
    print("🏥 MedAgg Healthcare POC - DEPLOYED AI BACKEND")
    print("=" * 70)
    ...
    try:
        ...
        with HTTPServer(('0.0.0.0', port), DeployedAIHandler) as httpd:
            print(f"🚀 Deployed AI backend server started on port {port}!")
            httpd.serve_forever()
    except Exception as e:
        print(f"❌ Error starting server: {e}")

if __name__ == "__main__":
    start_deployed_backend()
