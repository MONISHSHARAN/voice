#!/usr/bin/env python3
"""
MedAgg Healthcare Voice Agent - Hybrid HTTP/WebSocket Server
...
async def http_handler(request):
    ...
    logger.info(f"HTTP {method} request to {path}")
    
    if path == "/":
        return await home_page()
    elif path == "/twiml" and method == "POST":
        return await twiml_endpoint()
    elif path == "/test":
        return await test_page()
    elif path == "/register-patient" and method == "POST":
...
<Response>
    ...
    <Connect>
        <Stream url="wss://{PUBLIC_URL.replace('https://', '')}/twilio" />
    </Connect>
</Response>'''
...
<Response>
    <Say voice="alice">Hello! This is MedAgg Healthcare. I'm here to help you with your health concerns.</Say>
    <Hangup/>
</Response>'''
        return {
            'status': 200,
...
async def register_patient(request):
    ...
    try:
        ...
        # Make Twilio call
        call_success = await make_twilio_call(patient)
...
async def make_twilio_call(patient):
    """Make Twilio call"""
    try:
        # Create TwiML URL
...
async def hybrid_handler(request):
    ...
    if request.headers.get('upgrade') == 'websocket':
        ...
        await router(websocket, request.path)
    else:
        # HTTP connection
        return await http_handler(request)

def main():
    """Main function to start hybrid server"""
    ...
    # Start hybrid server
    logger.info("🌐 Starting hybrid HTTP/WebSocket server on port 5000")
    server = websockets.serve(hybrid_handler, "0.0.0.0", 5000)
