#!/usr/bin/env python3
"""
MedAgg Healthcare Voice Agent - Flask HTTP Server
Handles web interface, patient registration, and healthcheck.
Runs on port 5000.
"""
...
def home():
    ...
    <body>
        <div class="container">
            ...
            <div class="info">
                <h3>📞 Twilio Setup</h3>
                <p>Configure your Twilio phone number with a TwiML Bin:</p>
                <div style="background: #f8f9fa; padding: 10px; margin: 10px 0; border-radius: 3px; font-family: monospace;">
                    Voice URL: TwiML Bin URL<br>
                    HTTP Method: POST
                </div>
                <p><strong>TwiML Bin Content:</strong></p>
                <pre style="background: #f8f9fa; padding: 10px; margin: 10px 0; border-radius: 3px; font-family: monospace;">
&lt;?xml version="1.0" encoding="UTF-8"?&gt;
&lt;Response&gt;
    &lt;Say language="en"&gt;This call may be monitored or recorded.&lt;/Say&gt;
    &lt;Connect&gt;
        &lt;Stream url="wss://{{ public_url.replace('https://', '') }}:5001/twilio" /&gt;
    &lt;/Connect&gt;
&lt;/Response&gt;
                </pre>
                <a href="https://console.twilio.com/us1/develop/phone-numbers/manage/incoming" target="_blank" class="button">Configure Twilio Phone Number</a>
            </div>
...
def health_check():
    ...
    return jsonify({"status": "healthy", "message": "HTTP server is running"}), 200
...
def make_twilio_call(patient):
    """Make Twilio call"""
    try:
        # Twilio will use the TwiML Bin configured in the console
        # The TwiML Bin will point to the WebSocket server
        ...
        # The URL here is for the TwiML Bin, not directly to our app
        # We are just initiating the call, Twilio will fetch TwiML from the Bin
        call = twilio_client.calls.create(
            url=f"{PUBLIC_URL}/twiml-placeholder", # This URL is a placeholder, Twilio will use the TwiML Bin URL
...
if __name__ == '__main__':
    logger.info("MedAgg Healthcare Voice Agent - Flask HTTP Server Starting...")
